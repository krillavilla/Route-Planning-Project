...
void Render::Display(const std::vector<RouteModel::Node*> &path) {
    // Create a new output_surface
    ...
    m_Scale = static_cast<float>(std::min(display.dimensions().x(), display.dimensions().y()));
    m_PixelsInMeter = static_cast<float>(m_Scale / m_Model.MetricScale());
    m_Matrix = io2d::matrix_2d::create_scale({m_Scale, -m_Scale}) *
              io2d::matrix_2d::create_translate({0.f, static_cast<float>(display.dimensions().y())});

    // Draw the background
    display.paint(m_BackgroundFillBrush);

    // Draw all the regular map elements
    DrawLanduses(display);
    DrawLeisure(display);
    DrawWater(display);
    DrawRailways(display);
    DrawHighways(display);
    DrawBuildings(display);

    // Set the path to be displayed
    m_Model.path.clear();
    for (const auto &node: path) {
        m_Model.path.push_back(*node);
    }
    ...
    DrawStartPosition(display);
    DrawEndPosition(display);
